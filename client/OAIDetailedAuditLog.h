/**
 * Permit.io API
 *  Authorization as a service 
 *
 * The version of the OpenAPI document: 2.0.0
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * OAIDetailedAuditLog.h
 *
 * 
 */

#ifndef OAIDetailedAuditLog_H
#define OAIDetailedAuditLog_H

#include <QJsonObject>

#include "OAIAuditLogObjects.h"
#include "OAIRaw_Data.h"
#include <QDateTime>
#include <QJsonValue>
#include <QString>

#include "OAIEnum.h"
#include "OAIObject.h"

namespace OpenAPI {
class OAIRaw_Data;
class OAIAuditLogObjects;

class OAIDetailedAuditLog : public OAIObject {
public:
    OAIDetailedAuditLog();
    OAIDetailedAuditLog(QString json);
    ~OAIDetailedAuditLog() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(QString jsonString) override;

    QString getId() const;
    void setId(const QString &id);
    bool is_id_Set() const;
    bool is_id_Valid() const;

    OAIRaw_Data getRawData() const;
    void setRawData(const OAIRaw_Data &raw_data);
    bool is_raw_data_Set() const;
    bool is_raw_data_Valid() const;

    QJsonValue getInput() const;
    void setInput(const QJsonValue &input);
    bool is_input_Set() const;
    bool is_input_Valid() const;

    QJsonValue getResult() const;
    void setResult(const QJsonValue &result);
    bool is_result_Set() const;
    bool is_result_Valid() const;

    QJsonValue getContext() const;
    void setContext(const QJsonValue &context);
    bool is_context_Set() const;
    bool is_context_Valid() const;

    QString getAction() const;
    void setAction(const QString &action);
    bool is_action_Set() const;
    bool is_action_Valid() const;

    QDateTime getTimestamp() const;
    void setTimestamp(const QDateTime &timestamp);
    bool is_timestamp_Set() const;
    bool is_timestamp_Valid() const;

    QString getQuery() const;
    void setQuery(const QString &query);
    bool is_query_Set() const;
    bool is_query_Valid() const;

    QString getUserKey() const;
    void setUserKey(const QString &user_key);
    bool is_user_key_Set() const;
    bool is_user_key_Valid() const;

    QString getUserEmail() const;
    void setUserEmail(const QString &user_email);
    bool is_user_email_Set() const;
    bool is_user_email_Valid() const;

    QString getUserName() const;
    void setUserName(const QString &user_name);
    bool is_user_name_Set() const;
    bool is_user_name_Valid() const;

    QString getResourceType() const;
    void setResourceType(const QString &resource_type);
    bool is_resource_type_Set() const;
    bool is_resource_type_Valid() const;

    QString getTenant() const;
    void setTenant(const QString &tenant);
    bool is_tenant_Set() const;
    bool is_tenant_Valid() const;

    bool isDecision() const;
    void setDecision(const bool &decision);
    bool is_decision_Set() const;
    bool is_decision_Valid() const;

    QString getReason() const;
    void setReason(const QString &reason);
    bool is_reason_Set() const;
    bool is_reason_Valid() const;

    QString getPdpConfigId() const;
    void setPdpConfigId(const QString &pdp_config_id);
    bool is_pdp_config_id_Set() const;
    bool is_pdp_config_id_Valid() const;

    QString getEnvId() const;
    void setEnvId(const QString &env_id);
    bool is_env_id_Set() const;
    bool is_env_id_Valid() const;

    QString getProjectId() const;
    void setProjectId(const QString &project_id);
    bool is_project_id_Set() const;
    bool is_project_id_Valid() const;

    QString getOrgId() const;
    void setOrgId(const QString &org_id);
    bool is_org_id_Set() const;
    bool is_org_id_Valid() const;

    OAIAuditLogObjects getObjects() const;
    void setObjects(const OAIAuditLogObjects &objects);
    bool is_objects_Set() const;
    bool is_objects_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    QString id;
    bool m_id_isSet;
    bool m_id_isValid;

    OAIRaw_Data raw_data;
    bool m_raw_data_isSet;
    bool m_raw_data_isValid;

    QJsonValue input;
    bool m_input_isSet;
    bool m_input_isValid;

    QJsonValue result;
    bool m_result_isSet;
    bool m_result_isValid;

    QJsonValue context;
    bool m_context_isSet;
    bool m_context_isValid;

    QString action;
    bool m_action_isSet;
    bool m_action_isValid;

    QDateTime timestamp;
    bool m_timestamp_isSet;
    bool m_timestamp_isValid;

    QString query;
    bool m_query_isSet;
    bool m_query_isValid;

    QString user_key;
    bool m_user_key_isSet;
    bool m_user_key_isValid;

    QString user_email;
    bool m_user_email_isSet;
    bool m_user_email_isValid;

    QString user_name;
    bool m_user_name_isSet;
    bool m_user_name_isValid;

    QString resource_type;
    bool m_resource_type_isSet;
    bool m_resource_type_isValid;

    QString tenant;
    bool m_tenant_isSet;
    bool m_tenant_isValid;

    bool decision;
    bool m_decision_isSet;
    bool m_decision_isValid;

    QString reason;
    bool m_reason_isSet;
    bool m_reason_isValid;

    QString pdp_config_id;
    bool m_pdp_config_id_isSet;
    bool m_pdp_config_id_isValid;

    QString env_id;
    bool m_env_id_isSet;
    bool m_env_id_isValid;

    QString project_id;
    bool m_project_id_isSet;
    bool m_project_id_isValid;

    QString org_id;
    bool m_org_id_isSet;
    bool m_org_id_isValid;

    OAIAuditLogObjects objects;
    bool m_objects_isSet;
    bool m_objects_isValid;
};

} // namespace OpenAPI

Q_DECLARE_METATYPE(OpenAPI::OAIDetailedAuditLog)

#endif // OAIDetailedAuditLog_H
